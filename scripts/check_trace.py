...
def calc_cost(grid_info,netlist,io_placement,state):
    ...
    for block in netlist.name_to_block.values():
        for connection_name in block.connections:
            connection_weight = netlist.name_to_block[block.name].weights[connection_name]
            two_way_cost += connection_weight*math.pow((placement_dict[block.name].x - placement_dict[connection_name].x),2)
            two_way_cost += connection_weight*math.pow((placement_dict[block.name].y - placement_dict[connection_name].y),2)
    cost = (two_way_cost/2)
    return cost
########################################################################################
